...
from src.logic.get_params import get_number_params, get_str_params
from src.logic.param_adaptor import str_param_adaptor
from src.logic.print_result import print_result
from src.masks.masks import add_name_mask
from src.prompt.prompt import Prompt
from src.validator.output_json_validator import InfoVal, OutputVal
    ...
    try:
        while True:
            ...
            masked_logits_np = add_name_mask(
                selected_func_name, func_names, vocab_dict, prompt_logits_np
            )
        ...
        for param in func_params:
            for param_name, param_type in param.items():
                ...
                if param_type.type in ["number", "integer"]:
                    prompt, num_parameter = get_number_params(
                        llm, prompt, func_params, vocab_dict, param_type.type
                    )
                    parameters_dict[param_name] = num_parameter

                if param_type.type == "string":
                    prompt += '"'
                    prompt, str_parameter = get_str_params(
                        llm, prompt, func_params, vocab_dict
                    )
                    parameters_dict[param_name] = str_param_adaptor(
                        str_parameter
                        )
...
def get_output_info(path2jsons: PathValidator, init_prompt: str) -> int:
    ...
    try:
        ...
        print(
            ...
            "Please, check the file permissions/structure and try it again."
            f"{Colors.RESET.value}"
        )
        return 5
