...
def ft_until_value(value: int):
    for n in range(0, value + 1):
        yield f"Value {n}"

value = 100
generator = ft_until_value(value)
for n in range (0, value + 1):
    print(next(generator))
print(type(generator)) 
'''
Como la funcion tiene un yield, se genera una instancia
de la clase generator. Sobre este objeto es sobre el
que se tiene que iterar con next.
'''


'''
El método iter, convierte un iterable en un iterador para
poder usar next (solo actua sobre iteradores).
'''
my_list = [2 ,32, 45] #Es iterable pero no iterador
try:
    for n in range(len(my_list)):
        print(next(my_list))
except Exception:
    print("\nERROR: No se puede iterar sobre una lista sin con"
          "vertirla en iterador con iter()\n")
...
def esto_es_generador():
    if False:
        yield 1  # Nunca se ejecutará
    return 42

result = esto_es_generador()
print(type(result))  # <class 'generator'> ← ¡Sigue siendo generador!
